...
export async function listarUsuariosAtivos(limite = 50, lastUserId = null) {
  ...
  const { data: prefsPrioritarios, error: err1 } = await supabase
    .from("preferencias")
    .select("user_id, cargos_alvo, palavras_chave, regioes, modo_regiao, raio_km, modalidade_trabalho, salario_minimo, disparo_manual, busca_solicitada, ultima_busca_em")
    .eq("ativo", true)
